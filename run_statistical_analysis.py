...
Runs Group K-Fold CV on CMAPSS training data, then tests whether differences
in precision and ROC-AUC between models are statistically significant.

Usage:
    python run_statistical_analysis.py --dataset FD001
    python run_statistical_analysis.py --dataset FD001,FD002 --folds 5
...
PAIRS = [("RF", "XGBoost"), ("RF", "LSTM"), ("XGBoost", "LSTM")]
N_PAIRS = len(PAIRS)
ALPHA = 0.05
...
def _run_dataset(dataset: str, n_folds: int, cv_epochs: int) -> tuple[list[str], dict]:
    train_path = DATASETS[dataset]
    lines = []

    lines.append(f"\n{'='*64}")
    lines.append(f"Dataset: {dataset}  |  {n_folds}-fold Group CV  |  α={ALPHA}")
    lines.append(f"{'='*64}")
    ...
    results = {"RF": rf, "XGBoost": xgb, "LSTM": lstm}

    for metric in ("precision", "roc_auc"):
        scores = {m: results[m][metric] for m in MODELS}
        summary = compute_summary(scores)

        lines.append(f"\n── {metric.upper()} ──")

        # Mean ± std table
        header = f"  {'Model':<10}  {'Mean':>8}  {'Std':>8}  Per-fold scores"
        lines.append(header)
        for m in MODELS:
            s = summary[m]
        ...
        lines.append(
            f"\n  Friedman ({N_PAIRS+1} models, {n_folds} folds):  "
            f"chi2={fr['statistic']:.4f}  p={fr['p_value']:.4f}{_sig(fr['p_value'])}"
        )

        # Pairwise Wilcoxon
        lines.append(f"\n  Pairwise Wilcoxon (Bonferroni k={N_PAIRS}):")
        for m1, m2 in PAIRS:
        ...
        # Paired t-test
        lines.append(f"\n  Paired t-test (Bonferroni k={N_PAIRS}) [assumes normality]:")
        for m1, m2 in PAIRS:
    ...
    # McNemar (instance-level; same true labels shared across models)
    lines.append(f"\n── McNemar (pooled per-sample predictions across {n_folds} folds) ──")
    for m1, m2 in PAIRS:
        mc = mcnemar_test(
        ...
        lines.append(
            ...
            f"(n10={mc['n10']} A-right-B-wrong  n01={mc['n01']} A-wrong-B-right)"
        )

    lines.append(
        f"\n  Note: * = p < {ALPHA}.  With {n_folds} folds, power is limited — "
...
def main():
    parser = argparse.ArgumentParser(description="Statistical significance analysis for model comparison")
    ...
    parser.add_argument("--cv-epochs", type=int, default=20, help="LSTM epochs per fold (default: 20)")
    ...
    if unknown:
        print(f"ERROR: unknown dataset(s): {unknown}.  Valid: {list(DATASETS.keys())} or 'all'")
        sys.exit(1)
